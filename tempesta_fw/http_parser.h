...
typedef enum {
	/* Parsing headers. */
	TFW_HTTP_PARSE_HEADERS,
	/* Headers are parsed, parsing body. */
	TFW_HTTP_PARSE_BODY,
	/* Headers and body are parsed, parsing trailer. */
	TFW_HTTP_PARSE_TRAILER,
	/* The message is fully parsed. */
	TFW_HTTP_PARSE_DONE,
} tfw_http_parse_stage_t;

tfw_http_parse_stage_t tfw_http_parse_stage(TfwHttpMsg *hm);
